...
namespace epub2
{
    public static class ModifyInMemory
    {
        public static void ActivateMemoryPatching()
        {
            ...
            // var a = new Aspose.Pdf.Document();
            // var pg=a.Pages.Add();
// Bind input pdf file
             // converter.BindPdf("sample-document.pdf");
// // Initialize the converting process
//             converter.DoConvert();
            Assembly[] arr = AppDomain.CurrentDomain.GetAssemblies();
            var m = arr.Select(o => o.FullName).Where(o => o.Contains("aspose"));
        ...
        private static void ActivateForAssembly(Assembly assembly)
        {
            ...
            try
            {
                ...
                using (Stream stream = new MemoryStream(Convert.FromBase64String(LData)))
                {
                    ...
                    var cls = Activator.CreateInstance(type2, null);
                    mi.Invoke(cls, new Stream[] { stream });    
                }
